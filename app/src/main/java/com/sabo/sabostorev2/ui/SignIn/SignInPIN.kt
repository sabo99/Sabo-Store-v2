...
class SignInPIN : AppCompatActivity(), View.OnClickListener {
            ...
            mService!!.signInWithPIN(uid, PIN).enqueue(object : Callback<ResponseModel> {
                override fun onResponse(call: Call<ResponseModel>, response: Response<ResponseModel>) {
                    ...
                    if (code == 4) {
                        ...
                        Preferences.setUID(this@SignInPIN, userModel.uid)
                        Preferences.setIsLogIn(this@SignInPIN, true)
                        Preferences.setIsPIN(this@SignInPIN, true)

                        Handler().postDelayed({
                            sweetLoading.dismissWithAnimation()
                            startActivity(Intent(this@SignInPIN, HomeActivity::class.java))
                            finish()
                        }, 1000)
                    } else
